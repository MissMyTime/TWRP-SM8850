...
原理：解密前把 KeyMint 环境（OS 版本 / OS 补丁 / vendor 补丁）压制为已安装系统的真实值（支持 `twrp.neo8.osver/ospatch/venpatch` 属性覆盖），使密钥环境保持一致，不再触发不必要的升级。
...
注：与 `patches/nezha` 当前内容一致；后续按设备分别演进时请各自维护。
