...
#define GK_ERROR *gkResponse = GKResponse::error(), Status::ok()
	...
	bool usePersistentKeystoreDatabase() {
		static constexpr const char* source = "/data/misc/keystore";
		static constexpr const char* target = "/tmp/misc/keystore";
		struct stat st;
		if (stat("/data/misc/keystore/persistent.sqlite", &st) != 0) {
			printf("Keystore database is unavailable: %s\n", strerror(errno));
			return false;
		}
		...
		for (int i = 0; i < 50; ++i) {
			...
			if (!strcmp(service_state, "stopped"))
				break;
			usleep(100000);
		}
		if (strcmp(service_state, "stopped")) {
			printf("Unable to stop keystore2 before mounting its database\n");
			return false;
		}
		...
		if (umount2(target, MNT_DETACH) != 0 && errno != EINVAL && errno != ENOENT) {
			printf("Unable to detach the temporary keystore directory: %s\n", strerror(errno));
			return false;
		}
		if (mount(source, target, nullptr, MS_BIND, nullptr) != 0) {
			printf("Unable to bind the persistent keystore directory: %s\n", strerror(errno));
			return false;
		}
		...
		printf("keystore2 did not become ready with the persistent database\n");
		return false;
	}
...
extern "C" bool Decrypt_User(const userid_t user_id, const std::string& Password) {
	...
	if (!android::keystore::usePersistentKeystoreDatabase())
		return false;
    uint8_t *auth_token;
