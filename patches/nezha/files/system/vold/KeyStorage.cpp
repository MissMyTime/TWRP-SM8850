...
static KeystoreOperation BeginKeystoreOp(Keystore& keystore, const std::string& dir,
    ...
    // Recovery must never persist a KeyMint-upgraded blob. The installed OS
    // remains the sole owner of key upgrades and rollback metadata.
    std::lock_guard<std::mutex> lock(key_upgrade_lock);
...
//     }
    ...
    LOG(WARNING) << "KeyMint upgraded " << blob_file
                 << " for this recovery operation only; original blob is unchanged";
